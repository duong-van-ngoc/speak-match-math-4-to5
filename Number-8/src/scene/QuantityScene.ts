    ...
    objects: {
        // Vùng đặt item (tính theo boardContentRect, chừa phần vòng tròn phía dưới).
        areaPadX: 0.04,
        areaTop: 0.06,
        areaH: 0.58,
    ...
    perObject: {
        ...
        // Ví dụ:
        // [COUNT_AND_PAINT_ASSET_KEYS.stickyRoll]: { scaleMul: 1.15, offsetY: 8 },
        // Mặc định tăng giãn dọc (spreadYMul / gapYMul).
        [COUNT_AND_PAINT_ASSET_KEYS.squareCake]: {
            scaleMul: 1.6,
            offsetX: 0,
            offsetY: 20,
            spreadXMul: 1.2,
        ...
        [COUNT_AND_PAINT_ASSET_KEYS.watermelon]: {
            scaleMul: 1.6,
            offsetX: 0,
            offsetY: 0,
            spreadYMul: 1.5,
        ...
        [COUNT_AND_PAINT_ASSET_KEYS.lantern]: {
            scaleMul: 1.9,
            offsetX: 5,
            offsetY: 12,
            spreadYMul: 1.4,
        ...
        [COUNT_AND_PAINT_ASSET_KEYS.stickyRoll]: {
            scaleMul: 1.6,
            offsetX: 0,
            offsetY: 0,
            spreadYMul: 1.5,
        ...
        string,
        {
            ...
            maxScaleMul?: number; // giới hạn scale tối đa theo asset
...
export class CountAndPaintScene extends Phaser.Scene {
    ...
    // - Objects: COUNT_AND_PAINT_ASSET_KEYS.squareCake / watermelon / redEnvelope / lantern / stickyRoll
    // - Vòng tròn: COUNT_AND_PAINT_ASSET_KEYS.circleEmpty
        ...
        private levels: CountPaintLevel[] = [
            { objectKey: COUNT_AND_PAINT_ASSET_KEYS.watermelon, objectCount: 8 },
            { objectKey: COUNT_AND_PAINT_ASSET_KEYS.squareCake, objectCount: 8 },
            { objectKey: COUNT_AND_PAINT_ASSET_KEYS.redEnvelope, objectCount: 8 },
            { objectKey: COUNT_AND_PAINT_ASSET_KEYS.lantern, objectCount: 5 },
            { objectKey: COUNT_AND_PAINT_ASSET_KEYS.stickyRoll, objectCount: 6 },
        ];
    ...
    private createBoard() {
        ...
        this.board = this.add.image(0, 0, this.boardKey).setOrigin(0.5).setDepth(0);
            ...
            private drawObjects(level: CountPaintLevel) {
                ...
                const itemsRect = new Phaser.Geom.Rectangle(
                rect.x + rect.width * COUNT_AND_PAINT_TUNING.objects.areaPadX,
                rect.y + rect.height * COUNT_AND_PAINT_TUNING.objects.areaTop,
                rect.width * (1 - COUNT_AND_PAINT_TUNING.objects.areaPadX * 2),
                rect.height * COUNT_AND_PAINT_TUNING.objects.areaH
                );
                const areaW = itemsRect.width;
                const areaH = itemsRect.height;
                const centerX = itemsRect.centerX;
                const centerY = itemsRect.centerY;
                ...
                if (count === 5) {
                if (level.objectKey === COUNT_AND_PAINT_ASSET_KEYS.lantern) {
                    const rows = 2;
                    const cols = 3;
                    const gapX =
                    rect.width *
                    ...
                    const rowY = [startY, startY + (slotH + gapY)];
                    const stepX = slotW + gapX;

                    // Staggered 3-2 layout: bottom row centered under the top row.
                    positions.push(
                    { x: startX + 0 * stepX, y: rowY[0] },
                    { x: startX + 1 * stepX, y: rowY[0] },
                    { x: startX + 2 * stepX, y: rowY[0] },
                    { x: startX + 0.5 * stepX, y: rowY[1] },
                    { x: startX + 1.5 * stepX, y: rowY[1] }
                    );
                } else {
            ...
            const baseY = p?.y ?? centerY;
                ...
                const maxScaleMul = perObjTune.maxScaleMul ?? COUNT_AND_PAINT_TUNING.objects.maxScaleMul;
        ...
        private getCircleFillRatio(circle: Phaser.GameObjects.Image): number {
            ...
            private ensurePlaceholderTextures() {
                ...
                createRect(COUNT_AND_PAINT_ASSET_KEYS.squareCake, 0x60a5fa);
                createRect(COUNT_AND_PAINT_ASSET_KEYS.watermelon, 0x34d399);
                createRect(COUNT_AND_PAINT_ASSET_KEYS.redEnvelope, 0xfbbf24);
                createRect(COUNT_AND_PAINT_ASSET_KEYS.lantern, 0x38bdf8);
                createRect(COUNT_AND_PAINT_ASSET_KEYS.stickyRoll, 0x34d399);
