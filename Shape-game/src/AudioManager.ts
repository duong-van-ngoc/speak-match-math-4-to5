...
const SOUND_MAP: Record<string, SoundConfig> = {
  ...
  voice_guide_21: { src: `${BASE_PATH}level1.mp3`, volume: 0.8 }, // Level 1
  ...
  voice_guide_25: { src: `${BASE_PATH}level2.mp3`, volume: 0.8 }, // Level 2
...
const isVoiceLike = (id: string) => id.startsWith('voice_') || id.startsWith('prompt_') || id.startsWith('correct_answer_');
const isGuideVoice = (id: string) => GUIDE_VOICE_KEYS.includes(id);
...
class AudioManager {
  ...
  play(id: string): number | undefined {
    // If any other voice starts, stop guide voice so they don't overlap.
    if (id !== 'bgm_main' && isVoiceLike(id)) {
      this.stopGuideVoices(isGuideVoice(id) ? id : undefined);
    }
    if (this.unlocking) {
  ...
  stopGuideVoices(exceptId?: string): void {
    GUIDE_VOICE_KEYS.forEach((key) => {
