...
function getIrukaGame() {
    return (window as any).irukaGame as { retryFromStart?: () => void; prepareSubmitData?: () => unknown } | undefined;
}

function getSdk() {
    return (window as any).irukaSdk as { complete?: (payload: any) => void } | undefined;
}
...
export default class EndGameScene extends Phaser.Scene {
    ...
    create() {
        ...
        replayBtn.on('pointerdown', () => {
            ...
            this.scene.start('GameScene', { ...data, lessonId: this.lessonId, difficulty: this.difficulty });
            // Bật lại BGM sau khi chơi lại
            ensureBgmStarted();
            // SDK: tính số lần chơi lại
            getIrukaGame()?.retryFromStart?.();
        });
        ...
        exitBtn.on('pointerdown', () => {
            ...
            this.clearDimBackground();
            this.stopConfetti();
            // SDK: gửi complete khi thoát
            getSdk()?.complete?.({
                timeMs: Date.now() - ((window as any).irukaGameState?.startTime ?? Date.now()),
                extras: { reason: "user_exit", stats: getIrukaGame()?.prepareSubmitData?.() },
            });
