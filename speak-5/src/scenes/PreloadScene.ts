...
export default class PreloadScene extends Phaser.Scene {
    ...
    preload() {
        ...
        // Load hình ảnh phương tiện và chữ đi kèm từ config
        const vehicles = this.cache.json.get('game_config').vehicles;
        if (vehicles && Array.isArray(vehicles)) {
            vehicles.forEach((vehicle: any) => {
                ...
                // Add to AudioManager
                if (vehicle.audioKey && vehicle.audioPath) {
                    AudioManager.addSound(vehicle.audioKey, { src: vehicle.audioPath, volume: 1.0 });
                }
            });
        }
