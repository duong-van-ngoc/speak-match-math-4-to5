...
export default class CountGroupsDetailScene extends Phaser.Scene {
  ...
  // Manual tuning for placement per group (screen px; positive dx = right).
  // Image and text offsets are separate so we can adjust each independently.
  private readonly vehicleImageOffsetByGroup: Record<string, { dx: number; dy: number }> = {
    redPacket: { dx: 0, dy: 18 },
  };
  private readonly labelImageOffsetByGroup: Record<string, { dx: number; dy: number }> = {
    redPacket: { dx: 0, dy: 10 },
  };
  private readonly labelTextOffsetByGroup: Record<string, { dx: number; dy: number }> = {
    redPacket: { dx: 0, dy: 25 },
  };
  ...
  private getVehicleLabelAssetKey() {
    switch (this.initData?.groupId) {
      case 'watermelon':
        return COUNT_GROUPS_ASSET_KEYS.detailTextWatermelon;
      case 'squareCake':
        return COUNT_GROUPS_ASSET_KEYS.detailTextSquareCake;
      case 'redPacket':
        return COUNT_GROUPS_ASSET_KEYS.detailTextRedPacket;
      case 'lantern':
        return COUNT_GROUPS_ASSET_KEYS.detailTextLantern;
      case 'stickyRoll':
        return COUNT_GROUPS_ASSET_KEYS.detailTextStickyRoll;
      default:
        return undefined;
    }
  }

  private getLabelImageOffset() {
    const gid = String(this.initData?.groupId ?? '');
    return this.labelImageOffsetByGroup[gid] ?? { dx: 0, dy: 0 };
  }

  private getLabelTextOffset() {
    const gid = String(this.initData?.groupId ?? '');
    return this.labelTextOffsetByGroup[gid] ?? { dx: 0, dy: 0 };
  }

  private getVehicleImageOffset() {
    const gid = String(this.initData?.groupId ?? '');
    return this.vehicleImageOffsetByGroup[gid] ?? { dx: 0, dy: 0 };
  }
  ...
  private layoutScene() {
    ...
    const imgTarget = Math.min(cardW * 0.88, cardH * 0.64);
    const vehicleImageOffset = this.getVehicleImageOffset();
    this.vehicleImage.setPosition(cardX + vehicleImageOffset.dx, cardY - cardH * 0.18 + vehicleImageOffset.dy);
    ...
    const labelImageOffset = this.getLabelImageOffset();
    const labelTextOffset = this.getLabelTextOffset();
    if (this.vehicleNameImage) {
      this.vehicleNameImage.setPosition(cardX + labelImageOffset.dx, nameY + labelImageOffset.dy);
      const targetW = cardW * 0.9;
      const targetH = cardH * 0.2;
      try {
    ...
    this.vehicleNameText?.setPosition(cardX + labelTextOffset.dx, nameY + labelTextOffset.dy);
