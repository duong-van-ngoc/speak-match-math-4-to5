    ...
    // - Vehicles: pack `spriteKey` values (e.g. veh_watermelon, veh_square_cake, ...)
    ...
    init(data: { pack?: { groups: GroupViewDef[] } }) {
        ...
        data?.pack?.groups ??
        [
            { id: 'stickyRoll', label: 'bánh tét', count: 6, spriteKey: 'veh_sticky_roll', x: 260, y: 170, cols: 3 },
            { id: 'squareCake', label: 'bánh chưng', count: 8, spriteKey: 'veh_square_cake', x: 1020, y: 170, cols: 3 },
            { id: 'lantern', label: 'lồng đèn', count: 5, spriteKey: 'veh_lantern', x: 260, y: 560, cols: 3 },
            { id: 'redPacket', label: 'lì xì', count: 8, spriteKey: 'veh_red_packet', x: 1020, y: 560, cols: 2 },
        ];
    }
    ...
    create() {
        ...
        this.setPrompt('Con nối các nhóm có 8 phương tiện vào con xúc xắc ở giữa nhé');
        ...
        if (res.ok) {
            ...
            this.setFeedback('Chưa đúng. Con chỉ nối nhóm có 8 thôi nhé!');
            try {
    ...
    private createDiceTarget(cx: number, cy: number) {
        ...
        this.setFeedback('Con thả nhóm có 8 vào đây nhé');
        });
    }
    ...
    private layoutScene() {
        ...
        const scale = Math.min((w * 0.98) / tw, (h * 0.82) / th);
        ...
        const clusterOffsetY = -r.height * 0.05;
        this.diceRoot.setPosition(r.centerX, r.centerY + clusterOffsetY);
        this.diceHit.setPosition(r.centerX, r.centerY + clusterOffsetY);
        ...
        stickyRoll: { x: rightX, y: bottomY + clusterOffsetY },
        lantern: { x: rightX, y: topY + clusterOffsetY },
        squareCake: { x: leftX, y: topY + clusterOffsetY },
        redPacket: { x: leftX, y: bottomY + clusterOffsetY },
        };
    ...
    private createGroupView(g: GroupViewDef) {
        ...
        let iconMaxW = 300;
        let iconMaxH = 230;
        if (g.id === 'squareCake' || g.id === 'stickyRoll') {
        iconMaxW = Math.round(iconMaxW * 0.92);
        iconMaxH = Math.round(iconMaxH * 0.92);
        }
        if (g.id === 'redPacket') {
        iconMaxW = Math.round(iconMaxW * 1.28);
        iconMaxH = Math.round(iconMaxH * 1.28);
        }
        if (g.id === 'squareCake') {
        iconMaxW = Math.round(iconMaxW * 1.1);
        iconMaxH = Math.round(iconMaxH * 1.1);
        }
    ...
    private fitImageTo(img: Phaser.GameObjects.Image, maxW: number, maxH: number) {
        ...
        img.setScale(scale);
        (img as any).setRoundPixels?.(true);
    }
    }
