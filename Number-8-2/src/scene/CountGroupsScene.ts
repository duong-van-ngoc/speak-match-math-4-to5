...
export default class CountGroupsScene extends Phaser.Scene {
    ...
    // - Vehicles: COUNT_GROUPS_ASSET_KEYS.vehWatermelon / vehSquareCake / vehRedPacket / vehLantern / vehStickyRoll
    ...
    private readonly stage2Order = ['watermelon', 'squareCake', 'redPacket', 'lantern', 'stickyRoll'] as const;
    ...
    private groupsData: GroupViewDef[] = [
        { id: 'watermelon', label: 'dưa hấu',     count: 1, spriteKey: COUNT_GROUPS_ASSET_KEYS.vehWatermelon,     x: 240, y: 170 },
        { id: 'squareCake', label: 'bánh chưng',  count: 1, spriteKey: COUNT_GROUPS_ASSET_KEYS.vehSquareCake,    x: 1040, y: 170 },
        { id: 'redPacket', label: 'lì xì',        count: 1, spriteKey: COUNT_GROUPS_ASSET_KEYS.vehRedPacket,    x: 640, y: 360 },
        { id: 'lantern', label: 'lồng đèn',       count: 1, spriteKey: COUNT_GROUPS_ASSET_KEYS.vehLantern,    x: 240, y: 550 },
        { id: 'stickyRoll', label: 'bánh tét',    count: 1, spriteKey: COUNT_GROUPS_ASSET_KEYS.vehStickyRoll, x: 1040, y: 550 },
    ];
    ...
    create() {
        ...
        { yRatio: 0.09, scale: 0.52, scaleXMul: 0.75, textScaleXMul: 1, textScaleYMul: 1 }
        );
        ...
        // Gợi ý: bàn tay asset chỉ vào dưa hấu.
    ...
    private createGroupView(g: GroupViewDef) {
        ...
        // fit image to target box (uniform size across items)
        const targetW = 280;
        const targetH = 200;
        ...
        if (g.id === 'watermelon' || g.id === 'redPacket' || g.id === 'lantern') {
            img.setScale(img.scaleX * 1.12);
        }
        }
    ...
    private onGroupClicked(groupId: string) {
        this.clearHint();
        const allowFreePick = getReplayMode() === 'debug';
        if (!allowFreePick && this.flow.isCompleted(groupId)) {
        this.setFeedback('Nhóm này con làm xong rồi!');
        return;
        }

        // Enforce stage-2 order: finish watermelon -> squareCake -> redPacket -> lantern -> stickyRoll.
        const required = this.getNextStage2Target();
        if (!allowFreePick && required && groupId !== required) {
    ...
    private layoutScene() {
        ...
        // Layout: redPacket in center, 4 others in 4 corners.
        const r = this.boardContentRect;
        const cornerPadX = r.width * 0.14;
        const cornerPadY = r.height * 0.19;
        ...
        const topRowDy = -r.height * 0.08;
        const topRowDx = r.width * 0.14;
        const midY = r.centerY + r.height * 0.24;
        const posById: Record<string, { x: number; y: number }> = {
            watermelon: { x: leftX + topRowDx, y: topY + topRowDy },
            squareCake: { x: rightX - topRowDx, y: topY + topRowDy },
            redPacket: { x: leftX, y: midY },
            lantern: { x: r.centerX, y: midY },
            stickyRoll: { x: rightX, y: midY },
        };
        ...
        const ids = ['watermelon', 'squareCake', 'redPacket', 'lantern', 'stickyRoll'] as const;
    ...
    private ensurePlaceholderVehicleTextures() {
        ...
        ensure(COUNT_GROUPS_ASSET_KEYS.vehWatermelon, 0x97ceff); // dưa hấu
        ensure(COUNT_GROUPS_ASSET_KEYS.vehSquareCake, 0xffd294); // bánh chưng
        ensure(COUNT_GROUPS_ASSET_KEYS.vehRedPacket, 0xcff15e); // lì xì
        ensure(COUNT_GROUPS_ASSET_KEYS.vehLantern, 0xffc526); // lồng đèn
        ensure(COUNT_GROUPS_ASSET_KEYS.vehStickyRoll, 0xff7677); // bánh tét
    }
