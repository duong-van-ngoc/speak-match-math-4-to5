...
export default class CircleMarkScene extends Phaser.Scene {
    ...
    private buildHalfAssetContainer(
        ...
        const img = this.add.image(0, 0, textureKey).setOrigin(0.5).setScale(0.85);
    ...
    private onUp(pointer: Phaser.Input.Pointer) {
        if (this.phase === 'circle') {
            ...
            // "Cấm khoanh ở góc": Tăng vùng góc lên (từ 15% -> 25%) để tránh khoanh quá sát mép
            ...
            // TỰ ĐỘNG NHẬN DIỆN NỬA TRÁI HAY PHẢI
            const assetCenterX = this.boardRect.centerX;
            let leftCount = 0;
            pts.forEach(p => { if (p.x < assetCenterX) leftCount++; });
            const isOnLeft = leftCount > pts.length * 0.5;
            const bagIndex = isOnLeft ? 0 : 1;

            // Nới lỏng cho nhóm thuyền nhỏ (bên trái)
            const minSize = isOnLeft ? 150 : 180;
            if (wRaw < minSize || hRaw < minSize) {
                this.cameras.main.shake(120, 0.01);
                this.playWrongSound();
                this.circling = undefined;
                return;
            ...
            // KIỂM TRA BỔ SUNG: Vùng khoanh phải bao phủ được tọa độ X của nhóm thuyền tương ứng
            const targetSideX = isOnLeft ? this.objectPositions!.leftX : this.objectPositions!.rightX;
            const tolerance = isOnLeft ? 50 : 0; // Thuyền nhỏ cho phép nới lỏng thêm 50px
            if (minX > targetSideX + tolerance || maxX < targetSideX - tolerance) {
                this.cameras.main.shake(120, 0.01);
                this.playWrongSound();
                this.circling = undefined;
                return;
            ...
            const targetBag = this.bags[bagIndex] as Phaser.GameObjects.Container;

            // Kiểm tra xem vùng này đã được khoanh chưa
            if (this.locked.has(targetBag)) {
                // Nếu vùng này đã khoanh, tìm vùng còn lại xem có khớp không
                const otherBagIndex = 1 - bagIndex;
                const otherBag = this.bags[otherBagIndex] as Phaser.GameObjects.Container;
                if (this.locked.has(otherBag)) {
                    this.circling = undefined;
                    return;
                }
                // Nếu chưa khớp (ví dụ khoanh ở giữa hoặc nhầm sang vùng đã khoanh)
                this.cameras.main.shake(120, 0.01);
                this.playWrongSound();
                this.circling = undefined;
                return;
    ...
    private applyCurrentLevelToObjects() {
        ...
        this.bags.forEach((bagObj, index) => {
            ...
            if (tex && this.textures.exists(tex)) {
                ...
                const img = this.add.image(0, 0, tex).setOrigin(0.5).setScale(0.85);
                const frameW = img.width;
                const frameH = img.height;
                // Để tránh lộ đường cắt ở giữa, cho overlap 2px
                const overlap = 2;
    ...
    private layoutBoard() {
        ...
        // Thang số nằm trên cùng, cách mép trên board khoảng 40px
    ...
    private showGuideHand(first: boolean) {
        ...
        if (this.phase === 'circle') {
            ...
            const objSpacing = Math.min(this.boardInnerRect.width * 0.5, 300);
            let offsetX = (isLeft ? -1 : 1) * (objSpacing / 2);
            if (isLeft) offsetX -= 120;
            const offsetY = -60;
            const r = 130;
