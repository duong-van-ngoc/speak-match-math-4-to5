    ...
    perObject: {
        moon: { scaleMul: 1.15, offsetX: 0, offsetY: 20, spreadYMul: 1.1, gapYMul: 1.15 },
        sun: {
            scaleMul: 1.6,
            offsetX: 0,
            offsetY: 0,
...
export class CountAndPaintScene extends Phaser.Scene {
    ...
    // - Vòng tròn: COUNT_AND_PAINT_ASSET_KEYS.circleEmpty
        ...
        private levels: CountPaintLevel[] = [
            { objectKey: 'sun', objectCount: 7 },      // mặt trời
            { objectKey: 'moon', objectCount: 7 },     // mặt trăng
            { objectKey: 'rainbow', objectCount: 4 },  // cầu vồng
            { objectKey: 'cloud', objectCount: 5 },    // đám mây
            { objectKey: 'star', objectCount: 7 },     // ngôi sao
        ];
    ...
    private createBoard() {
        ...
        this.board = this.add.image(0, 0, this.boardKey).setOrigin(0.5).setDepth(0);
            ...
            if (count === 7 && level.objectKey === 'moon') {
                spreadY *= 1.26;
            }
            if (count === 7 && level.objectKey === 'sun') {
                spreadY *= 0.98;
            }
            if (count === 7 && level.objectKey === 'star') {
                spreadY *= 0.92;
            }
            ...
            // const isSunOrStar7 = isLayout7 && (level.objectKey === 'sun' || level.objectKey === 'star');
            const sunExtraY = isLayout7 && level.objectKey === 'sun' ? areaH * 0.08 : 0;
            const starExtraY = isLayout7 && level.objectKey === 'star' ? areaH * 0.04 : 0;
            const ignorePerItemOffsets = isLayout7 ? true : perObjTune.ignorePerItemOffsets;
            const ignorePerItemScaleMul = isLayout7 ? true : perObjTune.ignorePerItemScaleMul;
            ...
            for (let i = 0; i < count; i++) {
                ...
                const perObjDy = (perObjTune.offsetY ?? 0) + sunExtraY + starExtraY;
                ...
                if (level.objectKey === 'sun') scale *= 1.1;
                if (level.objectKey === 'star') scale *= 0.9;
                if (level.objectKey === 'moon') scale *= 1.3;
                if (level.objectKey === 'cloud') scale *= 1.12;
                if (level.objectKey === 'rainbow') scale *= 1.06;
        ...
        private getCircleFillRatio(circle: Phaser.GameObjects.Image): number {
            ...
            const isLayout7 = level?.objectCount === 7;
            ...
            const hexExtraY = isLayout7 ? 8 : 0;
            const moonExtraY = isLayout7 && level?.objectKey === 'moon' ? -6 : 0;
        ...
        createRect('moon', 0x60a5fa);
        createRect('sun', 0x34d399);
        createRect('rainbow', 0xfbbf24);
        createRect('cloud', 0x38bdf8);
        createRect('star', 0x34d399);
