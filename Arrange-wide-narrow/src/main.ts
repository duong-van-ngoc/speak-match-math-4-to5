...
function setViewportBg(url: string, position: string = "center center") {
  // Ưu tiên set BG lên game-container để chắc chắn không bị lớp khác che
  const target = document.getElementById(containerId) ?? document.body;
  (target as HTMLElement).style.backgroundImage = `url("${url}")`;
  (target as HTMLElement).style.backgroundRepeat = "no-repeat";
  (target as HTMLElement).style.backgroundSize = "cover";
  (target as HTMLElement).style.backgroundPosition = position;
