    ...
    // - Vehicles: pack `spriteKey` values (e.g. bánh trưng, đèn lồng, ...)
    ...
    create() {
        ...
        { yRatio: 0.08, scale: 0.52, fitText: true, textPaddingRatio: 0.2, bannerScaleXMul: 1 }
        );
        ...
        this.setPrompt('Con nối các nhóm có 9 hình vào con xúc xắc ở giữa nhé');
        ...
        if (res.ok) {
            ...
            this.setFeedback('Chưa đúng. Con chỉ nối nhóm có 9 thôi nhé!');
            try {
    ...
    private createDiceTarget(cx: number, cy: number) {
        ...
        this.setFeedback('Con thả nhóm có 9 vào đây nhé');
        });
    }
    ...
    private layoutScene() {
        ...
        const scale = Math.min((w * 0.98) / tw, (h * 0.84) / th);
        ...
        const topY = r.top + cornerPadY - 15;
        const bottomY = r.bottom - cornerPadY - 15;

      const posById: Record<string, { x: number; y: number }> = {
        carp_lantern: { x: leftX, y: topY },
        star_lantern: { x: rightX, y: topY },
        lantern: { x: leftX, y: bottomY },
        paper_lantern: { x: rightX, y: bottomY },
      };
    ...
    private createGroupView(g: GroupViewDef) {
        ...
        const sizeBoost = g.id === 'paper_lantern' ? 1.1 : 1;
        const frameBoost = g.id === 'paper_lantern' ? 1.1 : 1;
        this.fitImageTo(icon, iconMaxW * sizeBoost * frameBoost, iconMaxH * sizeBoost * frameBoost);
        }
