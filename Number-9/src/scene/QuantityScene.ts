    ...
    perObject: {
        carp_lantern: { scaleMul: 1.5, offsetX: 0, offsetY: 20, spreadXMul: 1.2, spreadYMul: 1.65, gapYMul: 1.45 },
        mooncake: {
            scaleMul: 1.6,
            offsetX: 0,
            offsetY: -6,
        ...
        string,
        {
            ...
            areaWMul?: number; // nới vùng chứa item theo chiều ngang
            areaHMul?: number; // nới vùng chứa item theo chiều dọc
...
export class CountAndPaintScene extends Phaser.Scene {
    ...
    // - Vòng tròn: COUNT_AND_PAINT_ASSET_KEYS.circleEmpty
        ...
        private levels: CountPaintLevel[] = [
            { objectKey: 'mooncake', objectCount: 9 }, // bánh trung thu
            { objectKey: 'carp_lantern', objectCount: 6 }, // đèn lồng cá chép
            { objectKey: 'star_lantern', objectCount: 9 }, // đèn ông sao
            { objectKey: 'lantern', objectCount: 9 }, // đèn lồng
            { objectKey: 'paper_lantern', objectCount: 8 }, // đèn lồng giấy xếp
        ];
    ...
    private createBoard() {
        ...
        this.board = this.add.image(0, 0, this.boardKey).setOrigin(0.5).setDepth(0);
            ...
            private drawObjects(level: CountPaintLevel) {
                ...
                const areaW = rect.width * COUNT_AND_PAINT_TUNING.objects.areaW * (perObjTune.areaWMul ?? 1);
                const areaH = rect.height * COUNT_AND_PAINT_TUNING.objects.areaH * (perObjTune.areaHMul ?? 1);
                ...
                // Quy tắc xếp:
                // - 9 item: 3 hàng x 3 cột
                // - 8 item: 2 hàng x 4 cột
                // - 7 item: 3–1–3 cong nhẹ, chỉ dãn dọc
                // - 5 item: xếp vòng tròn
                // - 6 item: 2 hàng x 3 cột
                // - 4 item: 2 hàng x 2 cột
                const isStaggeredNine = count === 9 && (level.objectKey === 'mooncake' || level.objectKey === 'star_lantern');
                const isLanternStaggeredNine = count === 9 && level.objectKey === 'lantern';
                if (isStaggeredNine || isLanternStaggeredNine) {
                    const rows = isLanternStaggeredNine ? 3 : 2;
                    const topCols = isLanternStaggeredNine ? 3 : 4;
                    const middleCols = isLanternStaggeredNine ? 4 : 0;
                    const bottomCols = isLanternStaggeredNine ? 2 : 5;
                    const maxCols = Math.max(topCols, middleCols, bottomCols);
                    const gapX = rect.width * COUNT_AND_PAINT_TUNING.objects.gapX * (perObjTune.gapXMul ?? 1);
                    const gapY = rect.height * COUNT_AND_PAINT_TUNING.objects.gapY * (perObjTune.gapYMul ?? 1);
                    slotW = (areaW - (maxCols - 1) * gapX) / maxCols;
                    slotH = (areaH - (rows - 1) * gapY) / rows;

                    const startY = centerY - areaH / 2 + slotH / 2;
                    const rowTopY = startY;
                    const rowMiddleY = startY + slotH + gapY;
                    const rowBottomY = startY + (slotH + gapY) * (rows - 1);

                    const rowTopStartX = centerX - ((topCols - 1) * (slotW + gapX)) / 2;
                    const rowMiddleStartX = centerX - ((middleCols - 1) * (slotW + gapX)) / 2;
                    const rowBottomStartX = isLanternStaggeredNine
                        ? rowMiddleStartX + (slotW + gapX) / 2
                        : centerX - ((bottomCols - 1) * (slotW + gapX)) / 2;

                    for (let c = 0; c < topCols; c++) {
                        positions.push({ x: rowTopStartX + c * (slotW + gapX), y: rowTopY });
                    }
                    if (isLanternStaggeredNine) {
                        for (let c = 0; c < middleCols; c++) {
                            positions.push({ x: rowMiddleStartX + c * (slotW + gapX), y: rowMiddleY });
                        }
                    }
                    if (isLanternStaggeredNine) {
                        for (let c = 0; c < bottomCols; c++) {
                            positions.push({ x: rowBottomStartX + c * (slotW + gapX) * 2, y: rowBottomY });
                        }
                    } else {
                        for (let c = 0; c < bottomCols; c++) {
                            positions.push({ x: rowBottomStartX + c * (slotW + gapX), y: rowBottomY });
                        }
                    }
                    ...
                    // 7 item: xếp 3–1–3, cong nhẹ, dãn dọc nhiều hơn
                    const colX = [-1, 0, 1]; // trái, giữa, phải
                    const xSpacing = areaW * 0.3;
                    const ySpacing = areaH * 0.48;
                    const centerY7 = centerY + areaH * 0.04;
                    const curveY = areaH * 0.085;
                    // Hàng trên
                    for (let c = 0; c < 3; c++) {
                        positions.push({
                            x: centerX + colX[c] * xSpacing,
                            y: centerY7 - ySpacing + Math.abs(colX[c]) * curveY
                        });
                    }
                    // Hàng giữa
                    positions.push({ x: centerX, y: centerY7 });
                    // Hàng dưới
                    for (let c = 0; c < 3; c++) {
                        positions.push({
                            x: centerX + colX[c] * xSpacing,
                    ...
                    slotW = areaW * 0.42;
                    slotH = areaH * 0.38;
                    // Cap per-object scaleMul cho count=7
                    if (perObjTune && typeof perObjTune.scaleMul === 'number') {
                        perObjTune.scaleMul = Math.max(perObjTune.scaleMul, 1.16);
                    }
                } else if (count === 5) {
                    // 5 item: 4 góc + 1 giữa, thu hẹp dọc và tăng size
                    const rX = areaW * 0.33;
                    const rY = areaH * 0.24;
                    const centerY5 = centerY + areaH * 0.03;
                    positions.push(
                        { x: centerX - rX, y: centerY5 - rY },
                        { x: centerX + rX, y: centerY5 - rY },
                        { x: centerX - rX, y: centerY5 + rY },
                        { x: centerX + rX, y: centerY5 + rY },
                        { x: centerX, y: centerY5 }
                    );
                    slotW = areaW * 0.44;
                    slotH = areaH * 0.4;
                } else {
                    // ...existing code for grid layout...
            ...
            let spreadY = perObjTune.spreadYMul ?? 1;
            ...
            const isLayout7 = count === 7;
            const isLayout5 = count === 5;
            const isGridLayout = count >= 8;
            const ignorePerItemOffsets = isGridLayout || isLayout7 ? true : perObjTune.ignorePerItemOffsets;
            const ignorePerItemScaleMul = isGridLayout || isLayout7 ? true : perObjTune.ignorePerItemScaleMul;

            for (let i = 0; i < count; i++) {
                const p = positions[i];
                const baseX = p?.x ?? centerX;
                const baseY = p?.y ?? centerY;
                const perItemDx = ignorePerItemOffsets ? 0 : (COUNT_AND_PAINT_TUNING.perItem.offsetX[i] ?? 0);
                const perItemDy = ignorePerItemOffsets ? 0 : (COUNT_AND_PAINT_TUNING.perItem.offsetY[i] ?? 0);
                ...
                const perItemBoost =
                    level.objectKey === 'carp_lantern'
                        ? 1
                        : 1 + i * COUNT_AND_PAINT_TUNING.objects.perIndexScaleStep * indexScaleStepMul;
                const perItemMul =
                    level.objectKey === 'carp_lantern' || ignorePerItemScaleMul
                        ? 1
                        : (COUNT_AND_PAINT_TUNING.perItem.scaleMul[i] ?? 1);
                const perObjMul = perObjTune.scaleMul ?? 1;
                // Áp dụng layoutScaleMul cho layout 7
                let scale = base * COUNT_AND_PAINT_TUNING.objects.baseScaleMul * perItemBoost * perItemMul * perObjMul;
                if (isLayout7) scale *= 1.06;
                if (isLayout5) scale *= 1.06;
                if (level.objectKey === 'mooncake') scale *= 1.05;
                if (level.objectKey === 'paper_lantern') scale *= 1.15;
                if (level.objectKey === 'carp_lantern') scale *= 1.3;
                if (level.objectKey === 'lantern') scale *= 1.3;
                if (level.objectKey === 'star_lantern') scale *= 1.06;
                scale = Math.min(scale, base * COUNT_AND_PAINT_TUNING.objects.maxScaleMul);
        ...
        private getNumberAssetKey(n: number): string | null {
            ...
            case 7:
                return this.textures.exists(COUNT_AND_PAINT_ASSET_KEYS.number7) ? COUNT_AND_PAINT_ASSET_KEYS.number7 : null;
            case 8:
                return this.textures.exists(COUNT_AND_PAINT_ASSET_KEYS.number8) ? COUNT_AND_PAINT_ASSET_KEYS.number8 : null;
            case 9:
                return this.textures.exists(COUNT_AND_PAINT_ASSET_KEYS.number9) ? COUNT_AND_PAINT_ASSET_KEYS.number9 : null;
            default:
                return null;
            }
        }
        ...
        private getCircleFillRatio(circle: Phaser.GameObjects.Image): number {
            ...
            const isGrid = level?.objectCount === 4 || level?.objectCount === 6 || level?.objectCount === 8 || level?.objectCount === 9;
            const gridExtraY = isGrid ? COUNT_AND_PAINT_TUNING.countLabel.grid46ExtraY : 0;
            const gridScaleMul = isGrid ? COUNT_AND_PAINT_TUNING.countLabel.grid46ScaleMul : 1;
            ...
            const step = (i: number) => {
                ...
                const labelY =
                    s.y +
                    s.displayHeight * COUNT_AND_PAINT_TUNING.countLabel.yFromSprite +
                    labelOffsetY +
                    gridExtraY;
        ...
        createRect('carp_lantern', 0x60a5fa);
        createRect('mooncake', 0x34d399);
        createRect('star_lantern', 0xfbbf24);
        createRect('lantern', 0x38bdf8);
        createRect('paper_lantern', 0x34d399);
