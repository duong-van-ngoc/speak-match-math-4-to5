...
class AudioManager {
  ...
  async unlockAndWarmup(ids: string[] = ['sfx_click', 'sfx_correct', 'sfx_wrong', 'bgm_main']) {
  ...
  playStage1PaintPrompt(objectKey: string): void {
    const id =
      objectKey === 'mooncake'
        ? 'voice_stage1_paint_mooncake'
        : objectKey === 'carp_lantern'
          ? 'voice_stage1_paint_carp_lantern'
          : objectKey === 'star_lantern'
            ? 'voice_stage1_paint_star_lantern'
            : objectKey === 'lantern'
              ? 'voice_stage1_paint_lantern'
              : objectKey === 'paper_lantern'
                ? 'voice_stage1_paint_paper_lantern'
                : undefined;
    if (!id) return;
  ...
  playCountNumber(n: number): void {
    const num = Math.max(1, Math.min(9, Math.round(n || 1)));
    this.playVoiceInterrupt(`voice_count_${num}`);
  }

  playCountSequence(n: number): Promise<void> {
    const max = Math.max(1, Math.min(9, Math.round(n || 1)));
