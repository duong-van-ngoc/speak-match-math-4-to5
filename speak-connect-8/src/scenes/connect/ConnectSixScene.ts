...
export default class ConnectSixScene extends Phaser.Scene {
    ...
    private readonly lineStyle = {
        width: GameConstants.CONNECT_SCENE.LINE.WIDTH,
        color: GameConstants.CONNECT_SCENE.LINE.COLOR,
        alpha: GameConstants.CONNECT_SCENE.LINE.ALPHA
    };
    ...
    init() {
        ...
        this.groupsData = GameConstants.CONNECT_SCENE.GROUPS_DATA as any as GroupViewDef[];
    }
    ...
    create() {
        ...
        if (GameConstants.CONNECT_SCENE.TEST_MODE_CONNECT) {
            banner.setInteractive({ useHandCursor: true });
            banner.on('pointerdown', () => {
                console.log('[ConnectSixScene] TEST_MODE: Skipping scene');
                this.finishMinigameAfterCorrectVoice();
            });
        }
        ...
        if (GameConstants.IS_TEST_CONNECT_ONLY) {
            const TOTAL_STEPS = GameConstants.TET_ITEMS.TOTAL_LEVELS + this.flow.totalTargets;
        ...
        this.input.on('pointerup', (p: Phaser.Input.Pointer) => {
            ...
            if (res.ok) {
                ...
                const TOTAL_STEPS = GameConstants.TET_ITEMS.TOTAL_LEVELS + this.flow.totalTargets;
                sdk.progress((GameConstants.TET_ITEMS.TOTAL_LEVELS + this.flow.connectedTargets) / TOTAL_STEPS);
    ...
    private layoutScene() {
        ...
        const posById: Record<string, { x: number; y: number }> = {
            stickyRoll: { x: leftX, y: topY },
            squareCake: { x: rightX, y: topY },
            red: { x: leftX, y: bottomY },
            lantern: { x: rightX, y: bottomY },
        };
