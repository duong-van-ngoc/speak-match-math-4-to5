        ...
        try {
            ...
            if (!response.ok) {
                ...
                const detail = typeof errorData.detail === 'object'
                    ...
                    : errorData.detail;
                throw new Error(`Start session failed: ${detail}`);
            }
        ...
        try {
            const response = await fetch(url, {
                ...
                body: JSON.stringify({
                    ...
                    childId: request.learner_id, // Use childId for consistency
                    ageLevel: request.age_level, // Use ageLevel for consistency
                }),
                signal: AbortSignal.timeout(this.timeout),
            });
            ...
            if (!response.ok) {
                ...
                const detail = typeof errorData.detail === 'object'
                    ? JSON.stringify(errorData.detail)
                    : errorData.detail;
                throw new Error(`End session failed: ${detail}`);
            }
