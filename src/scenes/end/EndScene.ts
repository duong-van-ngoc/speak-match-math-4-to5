/**
 * EndGameScene - Màn kết thúc game
 * - Hiển thị confetti và âm thanh celebration
 * - Có nút Reset để chơi lại từ đầu
 * - Có nút Exit để thoát và gửi callback cho host
 */
...
export default class EndGameScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Banner
        this.add
            .image(w / 2, h / 2 - h * GameConstants.ENDGAME.UI.BANNER_OFFSET, 'banner_congrat')
            .setOrigin(0.5)
            .setDepth(100)
        ...
        if (this.textures.exists('icon')) {
            const icon = this.add.image(w / 2, h / 2 - GameConstants.ENDGAME.UI.ICON_OFFSET, 'icon');
            icon.setScale(0.5);
            icon.setDepth(1005);
            ...
            const replayBtn = this.add
                .image(w / 2 - spacing, h / 2 + h * GameConstants.ENDGAME.UI.BTN_OFFSET, 'btn_reset')
                .setOrigin(0.5)
                .setScale(btnScale)
                .setDepth(101)
            ...
            replayBtn.on('pointerdown', () => {
                ...
                this.stopConfetti();
                // Ensure to jump back to IntroScene or SpeakScene based on what is logical. 
                // Wait, SpeakScene seems like the first playable scene after Intro
                this.scene.start(SceneKeys.SpeakScene);
            });

            // Nút Exit (Bên phải)
            const exitBtn = this.add
                .image(w / 2 + spacing, h / 2 + h * GameConstants.ENDGAME.UI.BTN_OFFSET, 'btn_exit')
                .setOrigin(0.5)
                .setScale(btnScale)
                .setDepth(101)
                .setInteractive({ useHandCursor: true });
            ...
            exitBtn.on('pointerdown', () => {
                ...
                if (host && typeof host.complete === 'function') {
                    ...
                    // Fallback: nếu chạy ngoài Game Hub
                    console.log('[EndGame] No irukaHost found, game completed standalone.');
                }
            });
