// src/utils/rotateOrientation.ts
...
export function playVoiceLocked(
    ...
    if (key === 'voice-rotate') {
        ...
        if (currentVoiceKey === 'instruction' ||
            ...
            interruptedVoiceKey = currentVoiceKey; // Lưu lại tên voice bị ngắt
        }
        ...
        try {
            ...
            console.warn('[Rotate] Lỗi dừng audio:', e);
        }
...
function attachGlobalBlockInputListeners() {
    ...
    const handler = (ev: Event) => {
        ...
        // 2) Gọi phát voice-rotate (đã có cooldown bên trong)
        try {
            playVoiceLocked(null as any, 'voice-rotate');
        } catch (err) {
            console.warn(
                '[Rotate] Lỗi phát voice-rotate khi chạm overlay:',
                err
            );
        }
    };
...
function updateRotateHint() {
    ...
    const shouldShow = h > w && w < rotateConfig.breakpoint; // Dọc & nhỏ (mobile)
    ...
    // === Khi overlay BẬT LÊN LẦN ĐẦU ===
    if (overlayTurnedOn) {
        try {
            // Gọi voice-rotate ngay (bên trong đã có cooldown + stopAll)
            playVoiceLocked(null as any, 'voice-rotate');

            // Đóng băng mọi thứ: Tween, Timer, Update loop...
            ...
            console.warn('[Rotate] Lỗi tự động phát voice-rotate:', e);
        }
    }
...
/** Reset trạng thái voice (gọi khi chuyển scene) */
export function resetVoiceState() {
    currentVoiceKey = null;
    interruptedVoiceKey = null;
}

/** Lưu tham chiếu scene hiện tại (để pause/resume khi xoay) */
