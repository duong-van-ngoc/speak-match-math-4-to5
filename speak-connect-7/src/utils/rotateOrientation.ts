...
export function playVoiceLocked(
    ...
    if (key === 'voice-rotate') {
        ...
        currentVoiceKey === 'voice-speaking') {
    ...
    if (key === 'instruction' || key === 'cau_do' || key === 'voice_intro_s2' || key === 'voice-speaking') {
...
// Track orientation để phân biệt xoay màn hình vs resize thông thường
...
function updateRotateHint() {
    ...
    const currentOrientation: 'portrait' | 'landscape' = h > w ? 'portrait' : 'landscape';
    ...
    // Phân biệt xoay màn hình thực sự vs chỉ resize/chuyển tab
    const orientationChanged = lastOrientation !== null && lastOrientation !== currentOrientation;
    lastOrientation = currentOrientation;

    const overlayTurnedOn = !overlayWasActive && shouldShow;
    const overlayTurnedOff = overlayWasActive && !shouldShow && orientationChanged;
...
// === Khi overlay TẮT (xoay ngang lại) ===
    ...
    // --- LOGIC PHỤC HỒI MỚI ---
        if (interruptedVoiceKey) {
            ...
            // Xử lý đặc biệt cho voice-speaking (cần restart cả ngón tay + cancel timers)
            else if (interruptedVoiceKey === 'voice-speaking' && gameSceneReference.restartSpeaking) {
                gameSceneReference.restartSpeaking();
            }
            // Xử lý cho các voice khác (Chỉ cần đọc lại voice là đủ)
            else {
