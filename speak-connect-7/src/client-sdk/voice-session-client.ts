...
export class VoiceSessionClient {
        ...
        try {
            const response = await fetch(url, {
                ...
                body: JSON.stringify({
                    ...
                    age_level: request.ageLevel || "3-4",
            ...
            if (!response.ok) {
                ...
                console.error('[VoiceSessionClient] Start session error details:', JSON.stringify(errorData));
                throw new Error(`Start session failed: ${JSON.stringify(errorData)}`);
            }
    ...
    async submitAnswer(request: SubmitAnswerRequest): Promise<SubmitAnswerResponse> {
        ...
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                console.error('[VoiceSessionClient] Submit error details:', JSON.stringify(errorData));
                throw new Error(`Submit answer failed: ${JSON.stringify(errorData)}`);
            }
    ...
    async endSession(request: EndSessionRequest): Promise<EndSessionResponse> {
        ...
        try {
            const response = await fetch(url, {
                ...
                body: JSON.stringify({
                    total_questions_expect: request.totalQuestionsExpect,
                    is_user_aborted: request.isUserAborted,
            ...
            if (!response.ok) {
                const errorData = await response.json();
                console.error('[VoiceSessionClient] End session error details:', JSON.stringify(errorData));
                throw new Error(`End session failed: ${JSON.stringify(errorData)}`);
            }
