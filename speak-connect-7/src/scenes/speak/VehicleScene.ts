/**
 * NatureItemScene - Màn chơi đọc tên hiện tượng thiên nhiên
 * 
 * Flow:
 * 1. Hiển thị hình hiện tượng thiên nhiên
 * 2. Phát audio hướng dẫn "Con hãy đọc tên hiện tượng thiên nhiên này"
...
import * as voice from '../../client-sdk/voice-session-client'; // Sử dụng local SDK client để tránh CORS
...
export default class VehicleScene extends SceneBase {
    // UI Elements
    private natureImage!: Phaser.GameObjects.Image;
    private natureNameImage!: Phaser.GameObjects.Image;
    ...
    create() {
        ...
        // SDK Integration
        // Tổng cộng: 5 hiện tượng + 2 đường nối đúng = 7 bước
        const TOTAL_STEPS = GameConstants.NATURE_PHENOMENA.TOTAL_LEVELS + 2;
    ...
    protected createUI(): void {
        ...
        // 4. NATURE NAME IMAGE (Thay thế cho Text)
        const firstNatureItem = GameConstants.NATURE_PHENOMENA.ITEMS[0];

        this.natureNameImage = this.add.image(
            w * CFG.NATURE_TITLE.X,
            h * CFG.NATURE_TITLE.Y + 20, // Dịch xuống 1 chút
            (firstNatureItem as any).textKey
        )
            .setOrigin(0.5)
            .setScale(CFG.NATURE_TITLE.SCALE)
            .setDepth(10);

        // 5. NATURE IMAGE (sẽ thay đổi theo level)
        this.natureImage = this.add.image(
            w * CFG.NATURE_IMAGE.X,
            h * CFG.NATURE_IMAGE.Y + 20, // Dịch xuống 1 chút
            firstNatureItem.imageKey
        ).setScale(CFG.NATURE_IMAGE.SCALE).setDepth(10);

        // Floating animation
        this.tweens.add({
            targets: this.natureImage,
            y: (h * CFG.NATURE_IMAGE.Y + 20) - CFG.ANIM.FLOAT_DISTANCE,
    ...
    protected initGameFlow(): void {
        ...
        this.startWithAudio(async () => {
            ...
            AudioManager.play(GameConstants.NATURE_PHENOMENA.INTRO_AUDIO);

            const introDuration = AudioManager.getDuration(GameConstants.NATURE_PHENOMENA.INTRO_AUDIO) || 3;
    ...
    private onSpeakerClick(): void {
        ...
        // Phát audio mẫu
        const currentNatureItem = GameConstants.NATURE_PHENOMENA.ITEMS[this.currentLevel];
        AudioManager.play(currentNatureItem.audioKey);
        this.showSpeakAnimation();

        const audioDuration = AudioManager.getDuration(currentNatureItem.audioKey) || 2;
        this.time.delayedCall(audioDuration * 1000, () => {
            ...
            const micAudioKey = GameConstants.NATURE_PHENOMENA.MIC_AUDIO;
    ...
    private onMicroClick(): void {
        ...
        if (this.isSpeaking || this._isRecording || !this._isMicActivated) {
            // Visual feedback cho blocked click
            this.tweens.add({
                targets: this.microBtn,
    ...
    private async onRecordingComplete(audioBlob: Blob): Promise<void> {
        ...
        // Score the nature + Fake delay để bé kịp nhìn thấy mascot "đang nghĩ"
        const currentNatureItem = GameConstants.NATURE_PHENOMENA.ITEMS[this.currentLevel];

        // Chạy song song: Gọi API chấm điểm VÀ Chờ tối thiểu 2.5s
        const [score] = await Promise.all([
            this.submitForScoring(audioBlob, currentNatureItem.name),
    ...
    private async submitForScoring(audioBlob: Blob, keyword: string): Promise<number> {
        ...
        try {
            const audioFile = new File([audioBlob], `nature_${this.currentLevel + 1}.wav`, { type: 'audio/wav' });

            const sessionId = localStorage.getItem('voice_session_id') || '';
    ...
    private async goToNextLevel(): Promise<void> {
        ...
        if (this.currentLevel >= GameConstants.NATURE_PHENOMENA.TOTAL_LEVELS) {
    ...
    private updateLevelUI(): void {
        const currentNatureItem = GameConstants.NATURE_PHENOMENA.ITEMS[this.currentLevel];
        console.log(`[NatureItemScene] updateLevelUI: ${currentNatureItem?.name} (${currentNatureItem?.imageKey})`);

        this.tweens.add({
            targets: [this.natureImage, this.natureNameImage],
            alpha: 0,
            duration: 200,
            onComplete: () => {
                this.natureImage.setTexture(currentNatureItem.imageKey);
                if ((currentNatureItem as any).textKey) {
                    this.natureNameImage.setTexture((currentNatureItem as any).textKey);
                }

                this.natureImage.setAlpha(0);
                this.natureNameImage.setAlpha(0);
    ...
    private async startBackendSession(): Promise<void> {
        console.log('[VehicleScene] Starting backend session with local SDK client');
        ...
        try {
            ...
            if (response && response.sessionId) {
                ...
                console.log('[VehicleScene] Session started with ID:', response.sessionId);
                localStorage.setItem('voice_session_id', response.sessionId);
            } else {
                console.warn('[VehicleScene] StartSession response missing sessionId');
            }
        } catch (error) {
    ...
    private async endBackendSession(isUserAborted: boolean = false): Promise<void> {
        ...
        try {
            ...
            await voice.EndSession({
                sessionId: sessionId,
                totalQuestionsExpect: GameConstants.NATURE_PHENOMENA.TOTAL_LEVELS,
                isUserAborted,
                testmode: GameConstants.VOICE_RECORDING.TEST_MODE,
