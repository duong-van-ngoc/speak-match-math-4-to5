...
export class ColorScene extends Phaser.Scene {
  ...
  private playCorrectAnswerSound() {
    // Ngắt voice hướng dẫn trước khi phát âm thanh đúng (bé click nhanh tránh lồng tiếng)
  ...
  private playWrongSound() {
    // Ngắt voice hướng dẫn trước khi phát âm thanh sai (bé click nhanh tránh lồng tiếng)
    AudioManager.stopGuideVoices();
  ...
  private createDuckChickenObjects() {
    ...
    // Creates a single placeholder object that will be textured by the current level.
      ...
      const sprite = this.add.image(0, 0, firstKey).setInteractive().setScale(0.48);
