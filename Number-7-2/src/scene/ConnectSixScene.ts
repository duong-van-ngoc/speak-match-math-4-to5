    ...
    // - Vehicles: pack `spriteKey` values (e.g. sun, moon, ...)
    ...
    init(data: { pack?: { groups: GroupViewDef[] } }) {
        ...
        data?.pack?.groups ??
        [
            { id: 'moon', label: 'mặt trăng', count: 7, spriteKey: CONNECT_SIX_ASSET_KEYS.groupMoon7, x: 260, y: 170, cols: 3 },
            { id: 'cloud', label: 'đám mây', count: 5, spriteKey: CONNECT_SIX_ASSET_KEYS.groupCloud5, x: 1020, y: 170, cols: 3 },
            { id: 'rainbow', label: 'cầu vồng', count: 4, spriteKey: CONNECT_SIX_ASSET_KEYS.groupRainbow4, x: 260, y: 560, cols: 3 },
            { id: 'star', label: 'ngôi sao', count: 7, spriteKey: CONNECT_SIX_ASSET_KEYS.groupStar7, x: 1020, y: 560, cols: 3 },
        ];
    }
    ...
    create() {
        ...
        { yRatio: 0.09, scale: 0.52, fitText: true, textPaddingRatio: 0.2, bannerScaleXMul: 1 }
        );
        ...
        this.setPrompt('Con nối các nhóm có 7 hình vào con xúc xắc ở giữa nhé');
        ...
        if (res.ok) {
            ...
            this.setFeedback('Chưa đúng. Con chỉ nối nhóm có 7 thôi nhé!');
            try {
