...
export default class CountGroupsScene extends Phaser.Scene {
    ...
    // - Vehicles: COUNT_GROUPS_ASSET_KEYS.vehSun / vehMoon / vehRainbow / vehCloud / vehStar
    ...
    private readonly stage2Order = ['sun', 'moon', 'rainbow', 'cloud', 'star'] as const;
    ...
    private groupsData: GroupViewDef[] = [
        { id: 'sun', label: 'mặt trời', count: 7, spriteKey: COUNT_GROUPS_ASSET_KEYS.vehSun, x: 240, y: 170 },
        { id: 'moon', label: 'mặt trăng', count: 7, spriteKey: COUNT_GROUPS_ASSET_KEYS.vehMoon, x: 1040, y: 170 },
        { id: 'rainbow', label: 'cầu vồng', count: 5, spriteKey: COUNT_GROUPS_ASSET_KEYS.vehRainbow, x: 640, y: 360 },
        { id: 'cloud', label: 'đám mây', count: 4, spriteKey: COUNT_GROUPS_ASSET_KEYS.vehCloud, x: 240, y: 550 },
        { id: 'star', label: 'ngôi sao', count: 7, spriteKey: COUNT_GROUPS_ASSET_KEYS.vehStar, x: 1040, y: 550 },
    ];
    ...
    public replayStage2GuideAndPrompt() {
        ...
        if (!next) return;
        this.showHintAt(next, { forceVoice: true });
        })();
    }
    ...
    create() {
        ...
        { yRatio: 0.09, scale: 0.52, bannerScaleXMul: 0.85 }
        );
        ...
        // Gợi ý: bàn tay asset chỉ vào mặt trời.
        ...
        this.layoutScene();
        // Stage 2: play guide2 once after audio is unlocked, then guide the child step-by-step.
        const onAudioUnlocked = () => this.startStage2AudioFlow();
        window.addEventListener(AUDIO_UNLOCKED_EVENT, onAudioUnlocked, { once: true });
        this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
    ...
    private startStage2AudioFlow() {
        ...
        try {
            ...
            await AudioManager.waitForUnlock?.();
        } catch {}
    ...
    private createGroupView(g: GroupViewDef) {
        ...
        const hit = this.add
        .rectangle(0, 0, 280, 210, 0x000000, 0.001)
        .setOrigin(0.5)
    ...
    private onGroupClicked(groupId: string) {
        ...
        // Enforce stage-2 order: finish sun -> moon -> rainbow -> cloud -> star.
    ...
    private layoutScene() {
        ...
        const posById: Record<string, { x: number; y: number }> = {
            ...
            rainbow: { x: r.centerX, y: r.centerY },
            cloud: { x: leftX, y: bottomY },
            star: { x: rightX, y: bottomY },
        };
        ...
        const ids = ['sun', 'moon', 'rainbow', 'cloud', 'star'] as const;
    ...
    private showHintAt(groupId: string, opts?: { voice?: boolean; forceVoice?: boolean }) {
        this.moveHintHandTo(groupId);
        // Speak exactly when the hand points (only for the current required item).
        const allowVoice = opts?.voice !== false;
        const forceVoice = opts?.forceVoice === true;
        const required = this.getNextStage2Target();
        if (allowVoice && required && groupId === required) {
        const now = Date.now();
        const tooSoon = this.lastStage2HintVoiceGroupId === groupId && now - this.lastStage2HintVoiceAt < 2500;
        if (forceVoice || !tooSoon) {
    ...
    private ensurePlaceholderVehicleTextures() {
        ...
        ensure(COUNT_GROUPS_ASSET_KEYS.vehSun, 0x97ceff); // mặt trời
        ensure(COUNT_GROUPS_ASSET_KEYS.vehMoon, 0xffd294); // mặt trăng
        ensure(COUNT_GROUPS_ASSET_KEYS.vehRainbow, 0xcff15e); // cầu vồng
        ensure(COUNT_GROUPS_ASSET_KEYS.vehCloud, 0xffc526); // đám mây
        ensure(COUNT_GROUPS_ASSET_KEYS.vehStar, 0xff7677); // ngôi sao
    }
